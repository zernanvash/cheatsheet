# Tools Index

Command-focused cheatsheets organized by CTF challenge category. Start from [H4G Training](../H4G%20Training.md) for the full hub, or jump straight to a tool below.

> 🛠️ **Interactive Tool:** Use the [0xrefs Interactive Command Reference](../0xrefs.html) to search, filter, and dynamically inject environment variables into pentesting commands.

---

## 🖥️ Machine Exploitation

### Enumeration & Recon
- [Footprinting Cheat Sheet](Footprinting%20Cheat%20Sheet.md)
- [Passive Recon](Passive%20Recon.md)
- [Nmap Cheat Sheet](Nmap%20Cheat%20Sheet.md)
- [Dig Cheat Sheet](Dig%20Cheat%20Sheet.md)
- [Networking](Networking.md)
- [SMTP](SMTP.md)
- [SMBClient Cheat Sheet](SMBClient%20Cheat%20Sheet.md)
- [FTP Cheat Sheet](FTP%20Cheat%20Sheet.md)
- [Service Enumeration Alternatives](Service%20Enumeration%20Alternatives.md)
- [Cloud and Misc Recon Alternatives](Cloud%20and%20Misc%20Recon%20Alternatives.md)

### Exploitation & Initial Access
- [Metasploit](Metasploit.md)
- [EternalBlue Cheat Sheet](EternalBlue%20Cheat%20Sheet.md)
- [Password Attacks](Password%20Attacks.md)

### Privilege Escalation
- [Linux Attack Path Cheat Sheet](../guides/Linux%20Attack%20Path%20Cheat%20Sheet.md)
- [Windows Privilege Escalation Cheat Sheet](Windows%20Privilege%20Escalation%20Cheat%20Sheet.md)
- [Windows Attack Path Cheat Sheet](../guides/Windows%20Attack%20Path%20Cheat%20Sheet.md)

### Active Directory
- [Active Directory Attack Path Cheat Sheet](Active%20Directory%20Attack%20Path%20Cheat%20Sheet.md)
- [Kerberoasting Cheat Sheet](Kerberoasting%20Cheat%20Sheet.md)
- [Pass-the-Hash Cheat Sheet](Pass-the-Hash%20Cheat%20Sheet.md)
- [Mimikatz Cheat Sheet](Mimikatz%20Cheat%20Sheet.md)

### Post-Exploitation
- [Post-Exploitation](Post-Exploitation.md)

---

## 🌐 Web Exploitation

- [Web Testing](Web%20Testing.md)
- [Web Attack Alternatives](Web%20Attack%20Alternatives.md)

---

## ⚙️ Reverse Engineering (REV)

### Static Analysis (Disassemblers)
- [IDA Pro Cheat Sheet](IDA%20Pro%20Cheat%20Sheet.md)
- [Ghidra Cheat Sheet](Ghidra%20Cheat%20Sheet.md)
- [Rizin / Radare2 Cheat Sheet](Rizin%20Radare2%20Cheat%20Sheet.md)
- [Reversing CLI Tools Cheat Sheet](Reversing%20CLI%20Tools%20Cheat%20Sheet.md)
- [C Reversing Cheat Sheet](C%20Reversing%20Cheat%20Sheet.md)

### Dynamic Analysis (Debuggers)
- [GDB (gef) Cheat Sheet](GDB%20Cheat%20Sheet.md)
- [x64dbg Cheat Sheet](x64dbg%20Cheat%20Sheet.md)

### Solver & Scripting
- [REV Python Toolkit](REV%20Python%20Toolkit.md)
- [Vim For Reversing Cheat Sheet](Vim%20For%20Reversing%20Cheat%20Sheet.md)

---

## 💥 PWN (Binary Exploitation)

- [GDB (gef) Cheat Sheet](GDB%20Cheat%20Sheet.md) — breakpoints, pattern tools, canary, GOT, PIE
- [Reversing CLI Tools Cheat Sheet](Reversing%20CLI%20Tools%20Cheat%20Sheet.md) — checksec, ROPgadget, ropper, one_gadget
- [REV Python Toolkit](REV%20Python%20Toolkit.md) — pwntools exploit templates

---

## 🛡️ Forensics

- [Linux Text Processing](Linux%20Text%20Processing.md)

---

## 🔐 Cryptography & Password Attacks

- [Password Attacks](Password%20Attacks.md)

---

## 🔍 OSINT

- [Passive Recon](Passive%20Recon.md)
- [Dig Cheat Sheet](Dig%20Cheat%20Sheet.md)
- [Cloud and Misc Recon Alternatives](Cloud%20and%20Misc%20Recon%20Alternatives.md)

---

## 🗺️ General / Cross-Cutting

- [Wireless](Wireless.md)

---

## Blueprint Links

| Category | Blueprint |
|---|---|
| Machine Exploit | [Machine Exploitation Databank](../blueprints/Machine%20Exploitation%20Databank.md) |
| Machine Exploit | [Machine Attack Blueprint Index](../blueprints/machine-attacks/Machine%20Attack%20Blueprint%20Index.md) |
| Web | [Web Exploit Blueprint](../blueprints/Web%20Exploit%20Blueprint.md) |
| REV | [Reverse Engineering Blueprint](../blueprints/Reverse%20Engineering%20Blueprint.md) |
| PWN | [Buffer Overflow Blueprint](../blueprints/Buffer%20Overflow%20Blueprint.md) |
| Steg | [Steganography Blueprint](../blueprints/Steganography%20Blueprint.md) |
| Crypto | [Cryptography Blueprint](../blueprints/Cryptography%20Blueprint.md) |
| Mixed | [Challenge Use Cases](../references/Challenge%20Use%20Cases.md) |
